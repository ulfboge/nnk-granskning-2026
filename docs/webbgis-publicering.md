...
3. **Fältsynlighet — viktigast i denna del.** Högerklicka `NNK_naturaobjekt_yta` → *Data → Fields*. Slå PÅ *Visible* för **alla granskningsfält**: `granskat`, `justering`, `utbredning`, `livsmiljötyp1–3`, `kommentar_livsmil_utbred`, `tillstand`, `procent_gott`, `procent_ej_gott`, `procent_osaker`, `kommentar_tillstand`, `kontroll1–3`, `kommentar_kontroll`, `metod`, `Kommentar_metod` samt `lst_andrad_av`, `lst_andrad`. (Leveransens lyrx döljer flera av dem som standard för att hålla popupen kort — men **fält som är avstängda i lagret följer inte med i den publicerade tjänsten**, och då saknas de i WebbGIS-formuläret.) Behåll gärna avstängt: `nv_globalid`, `created_user/date`, `last_edited_user/date`, `habitat_priority_*`, `shape_Length/Area`. **Uppdatering 2026-09-22:** `habitat_period_*` (Startdatum/Slutdatum senaste inventering) ska nu istället slås PÅ — de har lagts till i popupens "Naturtyp (NNK-data)"-sektion (se `docs/popup-arcade-uttryck.md` avsnitt 2), och utan fältsynlighet här kommer de raderna alltid vara tomma i webbGIS trots att Arcade-koden finns. Gör detta vid nästa Share As Web Layer → Overwrite. **UPPDATERING 2026-09-22 (samma dag):** när fältsynlighet slås på och man försöker publicera ger Pro fel **00403 "Timestamp offset field is not supported for web feature layer"** — `habitat_period_lastdata_start`/`_end` är av Esri-fälttypen *Date, Time, and Timezone Offset* (`esriFieldTypeTimestampOffset`), som hostade feature layers/web feature layers inte stöder än. Det här förklarar sannolikt varför fälten från början hade satts till avstängda — någon hade sannolikt redan stött på samma problem tidigare, bara utan att skriva ner varför. **Åtgärd innan publicering fungerar:** ändra fälttypen till vanlig `Date` för båda fälten på alla tre lager (Fields-vyn i Pro → Data Type-kolumnen, eller geoprocessverktyget *Alter Field* med Field Type = Date) innan Share As Web Layer/Overwrite. OBS: eftersom fälten kommer från NNK Ajourhålla-uttaget återskapas de sannolikt som Timestamp Offset igen vid nästa uttag/ombyggnad av gdb:n — bäst är att skripta konverteringen i `forbered_gdb_for_publicering.py` så den sker automatiskt, istället för att göra om det manuellt i Pro varje gång. Spara (Ctrl+S i Fields-vyn). Upprepa för linje- och punktlagret. **UPPDATERING 2026-09-22 (bevarandeplan_ar):** ett nytt fält, `bevarandeplan_ar` (Bevarandeplan, fastställd (år)), har lagts till i samma popup-avsnitt. Det är ett vanligt heltalsfält (Long), inte Date/Timestamp Offset, så samma 00403-problem gäller inte det. Fältsynlighet för det sätts numera automatiskt av `jobbdator_bygg_nnk_lyrx_KORRIGERAD_V3.py` (via `EXTRA_VISIBLE_FIELDS`) — inget manuellt steg här behövs, förutsatt att lyrx-skriptet körs om efter att fältet finns i gdb:n (kräver i sin tur att `jobbdator_koppla_nnk_skyddskategori.py` körts om med `bevarandeplan_platser.csv` på plats, se kvarvarande_punkter_20260922.md).
